mtmm <-
function(datafile,x,color=FALSE,graph=FALSE,graphItem=FALSE)
{
...
V2<-c()
	for(l in 1:p2)
	{k<-1
mat1 <- matrix(ncol=1, nrow=((dim(X2[[l]])[2])*(dim(X2[[l]])[1])))
Var1 <- matrix(ncol=1, nrow=((dim(X2[[l]])[2])*(dim(X2[[l]])[1])))
Var2 <- matrix(ncol=1, nrow=((dim(X2[[l]])[2])*(dim(X2[[l]])[1])))
		...
		for (i in 1:(dim(X2[[l]])[1]))
		{
			...
			{
	mat1[k,1]<-X2[[l]][i,j]
	Var1[k,1]<-rep(attributes(X2[[l]])$dimnames[[1]][i],each=(dim(X2[[l]])[2]))[j]
...
for (k in 1:(nrow(err)))
	...
	for (l in 1:p1)
    {
    if(err$V2[k] %in% name[[l]]) i2[k]<-l
		if(err$V1[k] %in% name[[l]]) i1[k]<-l
    }
	}
...
#****************************************************************************
 if (graphItem==FALSE)
 {
	for (i in 1:p1)
    {
		...
		boxplot(Dim$correlation~Dim$mat,col=Y[[i]],main=paste("Scale",i,sep=" "),xlab="i",ylab=paste("Correlation of Items of scale i with Items of Scale",i),ylim = c(min(unlist(r2)), 1))
		}
 }
###############################################################################
...
tabl<-reshape(err,direction="wide",timevar="Scale",idvar="Item",v.names=c("correlation"))
...
par(mfrow=par.old)
	...
	for (i in 1:nbdim){
		...
		boxplot(Dim$correlation~Dim$ScaleI,col=Y[[i]],main=paste("Scale",i,sep=" "),ylim = c(min(unlist(r2)), 1),cex.axis=0.75,xlab="i",ylab=paste("Correlation of Items of scale i with Score Scale",i))
		}
 }
  if (graphItem==TRUE)
{
    ...
    Dim$Item <- factor(Dim$Item, levels=unique(as.character(Dim$Item)))
		with(Dim, stripchart(correlation ~ Item, vertical = TRUE, add =FALSE,
    main=paste("Score of Scale",i,sep=" "),xlab="Item",ylab="Correlation",ylim = c(min(unlist(r2)), 1),cex.axis=1,las=3,pch=20,cex=0.75))
